...
/*app.post("/users/edit/:id", function(req, res){
	db.users.find({_id: ObjectId(req.params.id)},  function(err, result){
		...
		var string = encodeURIComponent(result.id+'&first_name='+result.first_name+'&last_name='+result.last_name+'&email='+result.email);
...
app.post('/users/update/:id', function(req, res){
	db.users.update({_id: ObjectId(req.params.id)}, {$set:{first_name: req.body.first_name,last_name: req.body.last_name, email: req.body.email}}, function(err, result){
		if (err){
			console.log(err);
		}
		console.log('EDIT')
